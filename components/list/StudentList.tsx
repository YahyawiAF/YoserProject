...
import { deleteStudent, fetcher } from "../../utils/apiClient";
...
function StudentList({ setPage, setRow, title = "Student" }: any) {
  ...
  const { data, mutate } = useSWR(
    keyword
      ? `/api/student?page=${vpage - 1}&keyword=${keyword}`
      : `/api/student?page=${vpage - 1}`,
    fetcher
  );
  ...
  const editRecord = (id: string) => {
    setPage("edit");
    const dt =
      data && data.data && data.data.data?.find((m: any) => m.id === id);
    setRow({ ...dt });
  };
      ...
      <div className="py-4 px-4 w-full md:w-full m-5 md:mx-10 flex flex-col space-y-6 items-center bg-white border rounded-lg drop-shadow-sm shadow-gray-100">
        ...
        <div className="w-full">
          <table className="w-full border-collapse grid border-spacing-4 md:text-center">
            <tr
              className={`hidden md:grid md:grid-cols-8 bg-[#f9f5f1] font-bold text-yellow-900 text-sm tracking-wide rounded-lg mb-4 px-2 py-1`}
            >
              <td className="p-1 px-2">{title} ID</td>
              <td className="p-1 px-2">Student Name</td>
              <td className="p-1 px-2">Location</td>
              <td className="p-1 px-2">Phone</td>
              <td className="p-1 px-2">Class</td>
              <td className="p-1 px-2  md:text-center">Gender & Age</td>
              <td className="p-1 px-2  md:text-center">Guardian Name</td>
              <td className="p-1 px-2  md:text-center">&nbsp;</td>
            </tr>
            {data &&
              data.data &&
              data.data.data?.map((row: any, i: React.Key) => (
                <tr
                  key={i}
                  className={`grid md:grid-cols-8 text-gray-900 text-[0.81rem] font-normal font-circular rounded-md px-2 py-1 divide-y-3 border-b-2 border-slate-50`}
                >
                  <>
                    <td className="md:p-1 md:px-2">
                      <span className="px-4 py-1 w-full flex md:hidden rounded-md bg-[#f8f9fa] text-gray-500/70">
                        Student ID
                      </span>
                      <div className="px-2 py-1 indent-4 md:indent-0">
                        {row.refno}
                      </div>
                    </td>
                    <td className="md:p-1 md:px-2">
                    ...
                    <td className="md:p-1 md:px-2 md:text-center">
                      ...
                      <div className="px-2 py-1 indent-4 md:indent-0">
                        {row.gender} ({" "}
                        {moment().diff(moment(row.dob), "year") || 0} yrs )
                      </div>
                    </td>
                    <td className="md:p-1 md:px-2 md:text-center md:border-none border-dashed border-green-600">
                      <span className="mb-2 md:0 px-2 py-1 w-full flex md:hidden rounded-md bg-[#f8f9fa] text-gray-600/80">
                        Guardian Name
                      </span>
                      <div className="px-2 py-1 indent-4 md:indent-0">
                        {row.parent?.lname}, {row.parent?.fname}
                      </div>
                    </td>
                    <td className="md:p-1 md:px-2 md:text-center border-b-2 md:border-none border-dashed border-yellow-900/90">
                      ...
                      <Menu
                        as="div"
                        className="z-1 indent-6 md:indent-0 mb-4 md:0 relative"
                      >
                        <Menu.Button className="relative px-2 py-1 border-2 rounded">
                          <TfiMenuAlt className="z-20 h-4 w-4 text-gray-600/70" />
                          <Menu.Items
                            as="div"
                            className="z-10 absolute top-6 -left-0.5 md:-right-0.5 w-28 py-2 flex flex-col text-yellow-900/90 bg-white drop-shadow-sm border-2 rounded-bl rounded-br rounded-tl text-md text-gray-500 divide-y-2"
                          >
                            <Menu.Item
                              as="button"
                              onClick={() => editRecord(row.id)}
                              className="py-1 px-2 pl-3 flex space-x-4 items-center"
                            >
                              <FaUserEdit />
                              <span>Edit</span>
                            </Menu.Item>
